...
if not os.path.exists(CSV_FILE_esp_Soil_moisture_1):
    with open(CSV_FILE_esp_Soil_moisture_1, mode='w', newline='') as file:
        writer = csv.writer(file)
...
def get_data():
    """Fetches the latest sensor data for display."""
    try:
        data = {}
        if os.path.exists(CSV_FILE_esp_Temp_humid_1):
            with open(CSV_FILE_esp_Temp_humid_1, 'r') as file:
                last_line = file.readlines()[-1].strip()
                data['esp_Temp_humid_1'] = last_line.split(',')

        if os.path.exists(CSV_FILE_esp_Soil_moisture_1):
            with open(CSV_FILE_esp_Soil_moisture_1, 'r') as file:
                last_line = file.readlines()[-1].strip()
                data['esp_Soil_moisture_1'] = last_line.split(',')
